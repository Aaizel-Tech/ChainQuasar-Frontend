...
import { AlertTriangle, Search, Filter, Skull, Globe, Database, BarChart2, Calendar, Shield, Clock, Users, Download, RefreshCw } from 'lucide-react';
...
const recentAddressesData = [
  ...
  {
    address: "49oFBzfsTBgmgXSKN91WU8eJU4vJPhWDPiKjCCsxrwgTGRCvVDQHxqEGtdTFUQbZKJA1uKCceNitwpNbGwwdvwqN5s8GocE",
    type: "monero",
    category: "weapons",
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            <h1 className="text-lg font-semibold text-gray-900">Darkweb Monitoring</h1>
          </div>
      ...
      <main className="flex-1 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <h3 className="text-lg font-semibold text-gray-700">Darkweb Sites</h3>
                <p className="text-2xl font-bold">{statistics.totalSites.toLocaleString()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <h3 className="text-lg font-semibold text-gray-700">Crypto Addresses</h3>
                <p className="text-2xl font-bold">{statistics.totalAddresses.toLocaleString()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold">{statistics.sitesByCategory.length}</p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Addresses by Cryptocurrency</h2>
            <div className="h-64">
                ...
                <PieChart>
                  ...
                  >
                    {statistics.addressesByType.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={cryptoColors[entry._id] || '#9CA3AF'} />
                    ))}
                  </Pie>
                  <Tooltip 
                    formatter={(value) => [value, 'Addresses']}
                    labelFormatter={(index) => statistics.addressesByType[index]._id.charAt(0).toUpperCase() + statistics.addressesByType[index]._id.slice(1)}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Sites by Category</h2>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                ...
                >
                  ...
                  <Bar dataKey="count" name="Sites">
                    {statistics.sitesByCategory.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={categoryColors[entry._id] || '#9CA3AF'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Addresses by Site Category</h2>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
          ...
          {/* Crawl History Chart */}
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Crawl Discovery History</h2>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
                ...
                >
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="newSites" stroke="#8B5CF6" strokeWidth={2} name="New Sites" />
                  <Line type="monotone" dataKey="newAddresses" stroke="#3B82F6" strokeWidth={2} name="New Addresses" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow mb-6">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-700">Recently Discovered Addresses</h2>
                ...
                {recentAddresses.map((address, index) => (
                  <tr key={index}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600 hover:underline cursor-pointer">
                      {`${address.address.substring(0, 10)}...${address.address.substring(address.address.length - 4)}`}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        address.type === 'bitcoin' ? 'bg-amber-100 text-amber-800' :
                        address.type === 'ethereum' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        address.category === 'marketplace' ? 'bg-blue-100 text-blue-800' :
                        address.category === 'drugs' ? 'bg-green-100 text-green-800' :
                        address.category === 'weapons' ? 'bg-red-100 text-red-800' :
                        address.category === 'fraud' ? 'bg-amber-100 text-amber-800' :
                      ...
                      <div className="flex items-center">
                        <div className="w-full bg-gray-200 rounded-full h-2.5">
                          <div className={`h-2.5 rounded-full ${
                            address.confidence > 0.8 ? 'bg-green-600' :
                            address.confidence > 0.5 ? 'bg-yellow-400' :
                            'bg-red-600'
                          }`} style={{ width: `${address.confidence * 100}%` }}></div>
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <a href="#" className="text-purple-600 hover:text-purple-800">View Details</a>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="px-6 py-3 border-t border-gray-200 flex justify-between items-center">
            <p className="text-sm text-gray-700">
              Showing <span className="font-medium">5</span> of <span className="font-medium">142</span> addresses
            </p>
            <div>
              <button className="bg-gray-100 text-gray-700 px-3 py-1 rounded-md text-sm font-medium">Previous</button>
              <button className="bg-purple-600 text-white px-3 py-1 rounded-md text-sm font-medium ml-2">Next</button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow mb-6">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-700">Recently Crawled Sites</h2>
            <button className="text-purple-600 hover:text-purple-800 text-sm font-medium flex items-center">
              <Download className="h-4 w-4 mr-1" />
              Export
            </button>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Category</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Last Crawled</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {recentSites.map((site, index) => (
                  <tr key={index}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        site.category === 'marketplace' ? 'bg-blue-100 text-blue-800' :
                        site.category === 'hacking' ? 'bg-purple-100 text-purple-800' :
                        site.category === 'weapons' ? 'bg-red-100 text-red-800' :
                        site.category === 'fraud' ? 'bg-amber-100 text-amber-800' :
                        site.category === 'mixer' ? 'bg-pink-100 text-pink-800' :
          ...
          <div className="px-6 py-3 border-t border-gray-200 flex justify-between items-center">
            <p className="text-sm text-gray-700">
              Showing <span className="font-medium">5</span> of <span className="font-medium">{statistics.totalSites}</span> sites
            </p>
            <div>
              <button className="bg-gray-100 text-gray-700 px-3 py-1 rounded-md text-sm font-medium">Previous</button>
              <button className="bg-purple-600 text-white px-3 py-1 rounded-md text-sm font-medium ml-2">Next</button>
